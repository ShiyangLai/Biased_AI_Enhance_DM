...
dimension_bias_stats <- repdem_single_ai %>%
  dplyr::select(BiasSide, EngagementBehavioral, EngagementCognitive, EngagementEmotional, 
...
for(i in 1:length(dimensions)) {
  ...
  # Select whichever BiasSide contrast the fit produced. Hard-coding
  # "BiasSideSame" assumed "Opposite" was the reference level; upstream the
  # factor is ordered the other way, so the filter matched nothing and left
  # bias_coef with zero rows.
  bias_coef <- tidy(engagement_bias_model, conf.int = TRUE) %>%
    dplyr::filter(grepl("^BiasSide", term))
  stopifnot(nrow(bias_coef) == 1)
...
# The contrast below weights the four AIStanceLabel_S levels (Democrat,
# Republican, Neutral, Default), so the model must be fitted over that factor;
# it previously used the three-level BiasedType2, and emmeans then failed with
# "No variable named AIStanceLabel_S in the reference grid".
single_ai_processed$AIStanceLabel_S <- factor(single_ai_processed$AIStanceLabel_S)
per_model_ <- lm(PostPerformance ~ AIStanceLabel_S,
