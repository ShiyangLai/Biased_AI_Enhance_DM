...
calculate_marginal_means_mcmc <- function(model, data, variable = "BiasSide") {
  ...
  # UStanceLabel is in the fitted model, so it must appear here too or the
  # design matrix comes out narrower than the posterior. Held at its first
  # level: it is common to both rows and cancels in the contrast.
  ref_stance <- levels(factor(data$UStanceLabel))[1]

  pred_data_opposite <- data.frame(
    BiasSide = "Opposite",
    PrePerformance = ref_performance,
    NID = ref_nid,
    UStanceLabel = ref_stance
  )
  ...
  formula_fixed <- ~ BiasSide + as.factor(NID) + PrePerformance + as.factor(UStanceLabel)
  
  # Create full dataset for model matrix
  full_data <- rbind(pred_data_opposite, pred_data_same)
  # Inherit the fitted model's level order; hard-coding c("Opposite","Same")
  # made "Opposite" the reference here while the model used "Same", which
  # reverses the sign of the contrast.
  full_data$BiasSide <- factor(full_data$BiasSide, levels = levels(factor(data$BiasSide)))
  full_data$NID <- factor(full_data$NID, levels = levels(factor(data$NID)))
  full_data$UStanceLabel <- factor(full_data$UStanceLabel,
                                   levels = levels(factor(data$UStanceLabel)))
  
  X <- model.matrix(formula_fixed, data = full_data)
  stopifnot(identical(colnames(X), colnames(post_samples)))
...
summary_table <- emm_bias_df %>%
  ...
  dplyr::select(BiasSide, emmean, SE, CI_95)

cat("\n=== SUMMARY TABLE ===\n")
print(summary_table)

# Effect size calculation
effect_size <- abs(bias_pairwise_df$estimate[1])
